...
def init_logging(argdict, is_main=True):
    """Initilizes the logging and the SummaryWriter modules.

    If is_main is False (non-main DDP ranks), returns (None, None) and does not
    create log files to avoid multiple processes writing the same logs.
    """
    if not is_main:
        return None, None
